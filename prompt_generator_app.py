...
# 사용자 입력
problem = st.text_area("1. 문제 상황을 입력하세요", height=80)
decomposition = st.text_area("2. 문제를 어떻게 나눌 수 있을까요?", height=80)
pattern = st.text_area("3. 반복되는 패턴이 있나요?", height=80)
abstraction = st.text_area("4. 핵심 변수와 수식은 무엇인가요?", height=80)
algorithm = st.text_area("5. 해결을 위한 알고리즘은 어떤 순서인가요?", height=80)
